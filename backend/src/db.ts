...
const ensureGrmTables = async (): Promise<void> => {
  if (!grmTablesReady) {
    grmTablesReady = (async () => {
      ...
      if (Number(plaintesCountRows[0]?.total ?? 0) === 0) {
      ...
      if (Number(servicesCountRows[0]?.total ?? 0) === 0) {
...
const ensurePowerBITables = async (): Promise<void> => {
  if (!powerBITablesReady) {
    powerBITablesReady = (async () => {
      ...
      if (Number(reportsCountRows[0]?.total ?? 0) === 0) {
      ...
      if (Number(dashboardsCountRows[0]?.total ?? 0) === 0) {
...
const ensureOTTables = async (): Promise<void> => {
  if (!otTablesReady) {
    otTablesReady = (async () => {
      ...
      if (Number(profileRows[0]?.total ?? 0) === 0) {
        await getDbPool().execute(
      ...
      if (Number(activiteRows[0]?.total ?? 0) === 0) {
      ...
      if (Number(equipierRows[0]?.total ?? 0) === 0) {
      ...
      if (Number(rapportRows[0]?.total ?? 0) === 0) {
